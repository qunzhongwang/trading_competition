...
- **`data/`** — `LiveBuffer` is the central data store (asyncio.Lock-guarded deques) for candles and supplementary data (depth, funding, taker ratio). `WSConnector` (live) and `SimulatedFeed` (paper) are interchangeable candle producers. `BinanceSupplementaryFeed` collects free public Binance data (order book depth WS, mark price WS, taker ratio REST) in parallel.
...
- **`models/inference.py`** — `AlphaEngine` routes to rule-based scorer, LSTM via `ModelWrapper`, or ensemble. Mode selected by `alpha.engine` config value. Passes supplementary data through to extractor. Logs inference timing at DEBUG level.
- **`strategy/logic.py`** — One `StrategyLogic` instance per symbol. State machine transitions driven by alpha signals and order fills. Half-Kelly position sizing: scales allocation by alpha conviction (5-7.5%). Alpha-based order type: >0.85 → market, otherwise → limit. The monitor creates these and wires fill callbacks.
