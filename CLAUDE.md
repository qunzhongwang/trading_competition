...
- **`features/extractor.py`** — Stateless. Pure numpy, no pandas. Has two paths: `extract()` for single snapshot (rule-based), `extract_sequence()` for LSTM input `(seq_len, 10)` array with z-score normalization. 10 features: RSI, EMA fast/slow, ATR, momentum, volatility, order book imbalance, volume ratio, funding rate, taker ratio. Accepts optional `supplementary` dict for live data features.
- **`models/inference.py`** — `AlphaEngine` routes to rule-based scorer, LSTM via `ModelWrapper`, or ensemble. Mode selected by `alpha.engine` config value. Supports multi-TF filter (15m EMA crossover + 1h momentum) that dampens/boosts alpha. Per-symbol ICIR weights via optional `BayesianICIRTracker`. Logs inference timing at DEBUG level.
- **`models/icir_tracker.py`** — `BayesianICIRTracker` provides per-symbol adaptive factor weights. Online Bayesian shrinkage (`λ = min_lambda + (1-min_lambda) × exp(-n/tau)`) from offline ICIR priors toward observed IC. Fallback weights `[0.3, 0.3, 0.3, 0.1]`.
- **`strategy/logic.py`** — One `StrategyLogic` instance per symbol. Signal confirmation (N consecutive bars above threshold). State machine transitions driven by alpha signals and order fills. Half-Kelly position sizing (optionally adaptive via `TradeTracker`). Graduated exit tiers. Alpha-based order type: >0.85 → market, otherwise → limit. Alpha decay for stale signals.
- **`strategy/trade_tracker.py`** — `TradeTracker` records rolling trade outcomes for adaptive Kelly sizing. Blends observed win rate / payoff ratio with static priors.
- **`strategy/monitor.py`** — The orchestrator. Runs the full pipeline each time a closed candle arrives. Manages `MultiResampler`, stores higher-TF candles in buffer, feeds ICIR tracker with online observations, records trades for adaptive Kelly. Also runs `RiskShield.check_stops()` and circuit breaker checks every iteration.
...
- `"lstm"` — ONNX inference on 60-candle feature sequence (10 features, 128 hidden)
- `"ensemble"` — 50/50 average of both

### Position sizing

Uses Half-Kelly criterion scaled by alpha signal conviction:
- `base_size_pct: 0.05` (floor) to `max_size_pct: 0.15` (ceiling)
- `kelly_fraction: 0.5`, `estimated_win_rate: 0.55`, `estimated_payoff: 1.5`
- Alpha > `urgent_alpha_threshold: 0.85` → market order; otherwise → limit order
